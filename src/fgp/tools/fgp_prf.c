...
int decouple(bn_st * a, bn_st * b, char * input, uint8_t * key, int key_size)
{
	...
	// PRF on the input to get HASH_LEN pseudorandom bytes
	...
	// Separate the PRF output in hash_msg in two strings
	...
	// Expand each string to the appropriate size

	if (expand(exp_a, exp_a, HASH_LEN/2, EXP_LEN))		// The relevant initial size of exp_a is HASH_LEN/2
	{
	...
	if (expand(exp_b, exp_b, HASH_LEN/2, EXP_LEN))		// The relevant initial size of exp_b is HASH_LEN/2
	{
	...
	// Interpret the strings as integers and reduce their values with the order of the groups as modulus
